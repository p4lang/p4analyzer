...
use thiserror::Error;
use crate::json_rpc::{message::*, ErrorCode, from_json, DeserializeError};
...
impl<'machine> ProtocolMachine<'machine> {
  ...
  pub async fn process_request(&mut self, message: Message) -> Result<Option<Message>, ProtocolError> {
