...
use lsp_types::{lsif::*, Hover, HoverContents, MarkedString, NumberOrString, Url};
...
impl LsifWriter {
	...
	pub fn i32_to_enum(i: i32) -> NumberOrString { NumberOrString::Number(i) }

	pub fn data(&self) -> &LsifData { &self.data }
	...
	pub fn text_document_hover(&mut self) -> i32 {
		...
		let vertex = Entry {
			...
			data: Element::Vertex(Vertex::HoverResult {
				result: Hover {
					contents: HoverContents::Scalar(MarkedString::String("hi".to_string())),
					range: todo!(),
				},
			}),
		};
		...
		let edge = Entry { id, data: Element::Edge(Edge::Hover(EdgeData { in_v: todo!(), out_v: id })) };
