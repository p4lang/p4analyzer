...
use lsp_types::{Url, WorkspaceFolder, WorkspaceClientCapabilities, DidChangeWatchedFilesClientCapabilities, request::Shutdown};
...
impl LsifGenerator {
    ...
    async fn setup_workspace(&mut self) {
        ...
        initialize_params.workspace_folders = Some(vec![
            ...
            WorkspaceFolder{ uri: Url::from_directory_path(fs::canonicalize(&self.settings.header_files.as_ref().unwrap().clone()).expect("Failed to find System Header Folder")).unwrap(), name: "system_headers".into() }
        ]);
    ...
    async fn stop_workspace(&mut self) {
        ...
        // Force it to shutdown(doesn't work): cancel token
        self.token.cancel();
        ...
        // Nuke Self(does work): Doesn't print file as that is done after Async blocks
        //std::process::exit(0);
    }
