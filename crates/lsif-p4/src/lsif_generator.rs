use crate::{
	flags::LsifP4Cmd,
	lsif_writer::{LsifData, LsifWriter},
};
use analyzer_host::{
	json_rpc::message::{Message, Notification, Request, Response},
	lsp::workspace::{Workspace, WorkspaceManager},
	AnalyzerHost,
};
use cancellation::CancellationTokenSource;
use lsp_types::{DidChangeWatchedFilesClientCapabilities, Url, WorkspaceClientCapabilities, WorkspaceFolder};
use p4_analyzer::{
	driver::{buffer_driver::BufferStruct, Driver},
...
impl LsifGenerator {
	...
	fn data(&self) -> &LsifData { &self.writer.data() }
	...
	async fn setup_workspace(&mut self) {
		...
		initialize_params.workspace_folders = Some(vec![
			WorkspaceFolder {
				uri: Url::from_directory_path(
					fs::canonicalize(&self.settings.workspace.clone().unwrap_or_else(|| PathBuf::from(".")))
						.expect("Failed to find Workspace Folder"),
				)
				.unwrap(),
			...
			WorkspaceFolder {
				uri: Url::from_directory_path(
					fs::canonicalize(&self.settings.header_files.clone()).expect("Failed to find System Header Folder"),
				)
				.unwrap(),
				name: "system_headers".into(),
			},
		]);
		...
		self.driver
			.as_ref()
			.unwrap()
			.send_messages(queue![initialize_request, initialized_notification])
			.await;
		// results
