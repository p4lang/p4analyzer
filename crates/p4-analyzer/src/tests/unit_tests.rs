mod main_tests {
    ...
    use crate::get_logfile_stem;
    use crate::{cli::flags::{P4AnalyzerCmd, self, P4Analyzer}, create_default_logging_layer};
...
mod stdio_tests {
    ...
    async fn test_console_driver() {
		...
		let test_future = async{ driver.get_message_channel().0.clone().close(); };
		let (res, _) = tokio::join!(future, test_future);
		assert!(!res.is_err());

		let future = driver.start(token.token().clone());
		let test_future = async{ token.cancel(); };
		let (res, _) = tokio::join!(future, test_future);
		assert!(res.is_err());
	} 
	/*
...
mod lsp_server_tests {
    ...
    use crate::{commands::{lsp_server::LspServerCommand, Command}, cli::flags::Server};
